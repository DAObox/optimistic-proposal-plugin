...
import {ProposalDetails, ProposalStatus, DisputeStatus} from "../types/Types.sol";
import {InvalidProposalUpdate} from "../types/Errors.sol";
...
abstract contract Proposals is IProposals, ERC165 {
    ...
    function _updateProposal(uint256 _proposalId, bytes32 _what, bytes memory _value) internal {
        ...
        if (_what == "disputeStatus")
            proposal.disputeStatus = DisputeStatus(abi.decode(_value, (uint256)));
        else if (_what == "status") proposal.status = ProposalStatus(abi.decode(_value, (uint256)));
        else if (_what == "executionFromTime")
            proposal.executionFromTime = abi.decode(_value, (uint256));
        ...
        else if (_what == "proposerCollateral")
            proposal.proposerCollateral = abi.decode(_value, (uint256));
        else if (_what == "proposerPaidFees")
            proposal.proposerPaidFees = abi.decode(_value, (uint256));
        else if (_what == "challenger") proposal.challenger = abi.decode(_value, (address));
        else if (_what == "challengerPaidFees")
            proposal.challengerPaidFees = abi.decode(_value, (uint256));
    ...
    function _executeProposal(
        ...
        (execResults, failureMap) = _dao.execute(
            bytes32(_proposalId),
            proposal.actions,
    ...
    function supportsInterface(
        ...
        return
            _interfaceId == type(IProposals).interfaceId || super.supportsInterface(_interfaceId);
    }
}
