...
import {ArbitrableProposal} from "./ArbitrableProposal.sol";
...
contract OptimisticProposalPlugin is ArbitrableProposal, PluginCloneable {
  ...
  bytes32 public constant CONFIGURE_PARAMETERS_PERMISSION_ID = keccak256("CONFIGURE_PARAMETERS_PERMISSION_");
  ...
  function initialize(
    ...
    __ArbitrableProposal_init(_executionDelay, _arbitrator, _proposalCollateral, _metaEvidence, _arbitratorExtraData);
  }

  function updateState(bytes32 what, bytes calldata value) external auth(CONFIGURE_PARAMETERS_PERMISSION_ID) {
  ...
  function supportsInterface(bytes4 _interfaceId) public view override(PluginCloneable, Proposals) returns (bool) {
    return
