...
abstract contract ArbitrableProposal is IArbitrableProposal, Proposals, Initializable {
  ...
  function withdrawCollateral(uint256 amount) external payable returns (bool success) {
    ...
    if (amount < freeCollateral) revert InsufficientCollateral({requested: amount, available: freeCollateral});
  ...
  function cancelProposal(uint256 _proposalId) external {
    ...
    if (proposal.status != ProposalStatus.Active || proposal.status != ProposalStatus.RuledAllowed)
    ...
    proposerFreeCollateral[proposal.proposer] += proposal.proposerCollateral + proposal.proposerPaidFees;
  ...
  function _rule(uint256 _disputeID, uint256 _ruling) internal {
    ...
    if (proposal.disputeStatus != DisputeStatus.DisputeCreated) revert ProposalNotPaused(proposal.status);
    ...
    if (_ruling == 0) {
      ...
      payable(proposal.challenger).transfer(proposal.proposerCollateral + proposal.challengerPaidFees);
      emit ProposalRuled(proposalId, _disputeID, _ruling);
    } else {
